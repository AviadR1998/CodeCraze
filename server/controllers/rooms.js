...
const removeRoom = async (req, res) => {
    ...
    if (roomsList.get(req.params.username) !== "--") {
        ...
        getKey(arrSoc, player2).emit("removeRoom");
...
const joinRoom = async (req, res) => {
    ...
    if (roomsList.get(req.params.username) === "--") { //could be sync problem
        ...
        console.log(roomsList);
        getKey(arrSoc, req.params.username).emit('join', req.body.player);
        res.status(200);
    } else {
        res.status(400);
    }
...
const startGame = async (req, res) => {
    ...
    getKey(arrSoc, req.params.username).emit('obs', objs);
    let p2 = roomsList.get(req.params.username);
    //console.log(getKey(arrSoc, p2));
    getKey(arrSoc, p2).emit('obs', objs);
    res.status(200);
    res.end();
}
